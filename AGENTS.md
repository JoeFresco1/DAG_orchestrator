...
### Fallback across tools (chains)

Instead of hand-writing a command, point a task at a harness preset and give it
an ordered chain — attempt 1 uses the first tool, attempt 2 the next:

```bash
dag harness list                                       # what is installed
dag set --all --harness-chain "opencode:provider/model:xhigh,codex,claude:sonnet"
dag settings --harness-chain "opencode,codex"          # run-wide default
```

- The chain owns the command: presets supply `cmd`/`planCmd`, each candidate
  supplies its own model (`harness:model:variant`; model ids are
  provider-specific, so never reuse one across tools).
...
- `dag show` and the viewer mark which candidate the last attempt used
  (`harness`), so a fallback is visible in the run's history.
...
Some work is not "implement X" but "review what those tasks did". A chain
review task covers other tasks; its instruction is generated from their real
state when it runs.

```bash
dag layers                                   # dependency waves in this run
dag chain-review --wave 3 --batch 10         # one wave, 10 tasks per reviewer
dag chain-review --from <id> --depth 2       # a task and its dependency cone
dag chain-review --all --batch 25            # 500 tasks -> 20 reviewers
```

- It depends on what it covers (so it runs after them) and is a normal graph
  node: visible, retryable, skippable.
- The spec is a template rendered at run time: `{coverage}`,
  `{coverageManifest}`, `{coverageDir}`, `{coverageStat}`,
  `{coverageFiles}`. A bundle with a manifest and one diff per covered task is
  written to `dag.run.d/reviews/chain-<id>/` so a reviewer can work
  incrementally.
- Its VERDICT decides: FAIL fails it with kind `review` and feeds the reason
  into `{lastRejection}` for a retry. Chain tasks are never themselves
  reviewed by the end-of-run review.

Use it for cross-task problems: each task locally right, the combination wrong.
